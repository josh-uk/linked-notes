...
export async function captureWorkspaceManifest(createdAt = new Date()) {
  const snapshot = await prisma.$transaction(
    async (transaction) => {
      const [
        ...
        transaction.noteRevision.findMany({ orderBy: { id: "asc" } }),
        transaction.noteTemplate.findMany({ orderBy: { id: "asc" } }),
  ...
  const manifest: BackupManifest = {
    ...
    entities: {
      ...
      noteRevisions: snapshot.noteRevisions.map((revision) => ({
        id: revision.id,
        noteId: revision.noteId,
        noteVersion: revision.noteVersion,
        title: revision.title,
        content:
          revision.content as BackupManifest["entities"]["noteRevisions"][number]["content"],
        contentText: revision.contentText,
        reason: revision.reason,
        createdAt: revision.createdAt.toISOString(),
      })),
      noteTemplates: snapshot.noteTemplates.map((template) => ({
        id: template.id,
        name: template.name,
        title: template.title,
        content:
          template.content as BackupManifest["entities"]["noteTemplates"][number]["content"],
