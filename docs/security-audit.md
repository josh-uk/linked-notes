...
| Markdown and Apple migration  | Relative paths, file counts, text, metadata, editor JSON, source IDs, and destination state are bounded; imports preview before one serializable transaction     | Parser and real-PostgreSQL tests cover metadata, link remapping, idempotency, injection text, and nested folders      |
